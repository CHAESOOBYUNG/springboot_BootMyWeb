...
public class ProductServiceImpl implements ProductService {
	...
	//업로드 할 경로(application.properties값을 강조)
	...
	//multi insert작업 (상품 insert -> 파일 업로드 -> 업로드 테이블 insert)
	//transction라이브러리 필요(부트에선 포함됨)
	...
	public int regist(ProductVO vo, List<MultipartFile> list) {
		...
		for(MultipartFile f : list) {
			//1. 파일명 추출(브라우저 별로 다를 수 있기 때무넹 \\기준으로 파일명 추출)
			...
			//mybatis에 selectKey기능을 사용. (마이바티스에서 insert전 후에 특정테이블의 키를 구하느 기능)
